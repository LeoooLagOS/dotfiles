...
## 🔑 Key Details
- **What it is:** An unordered collection of items where every item must be unique. It's essentially a [[Hash Maps|Hash Map]] where we only care about the keys, not the values. 
- **Why it exists:** It is the most efficient data structure for solving problems related to uniqueness, such as removing duplicates from a list or checking if an item has been seen before.
## 🔗 Connections & Implementations
- Implemented in [[Java]] as the `HashSet` class. 
- Implemented in [[Python]] as the `set` object. 
